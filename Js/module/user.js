...
export const addUser = async (arg) => {
    ...
    let res = await fetch(`${enlace.user}`, config);
...
export const getUser = async (userId) => {
    ...
    let res = await fetch(`${enlace.user}/${userId}`);
    
    if (!res.ok) {
        return { status: res.status, message: "User not found" };
    }
...
export const deleteUser = async (arg) => {
    ...
    let res = await fetch(`${enlace.user}/${arg.id}`, config)
    if (res.status === 404) {
...
export const updateUser = async (id) => {
        ...
        if (user.status == 404) {
            alert(`Error ${user.status} => User not found`)
            return;
        }
        else {
            ...
            let selectedKey = options[parseInt(opc) - 1]; // obtener el indice del array restando 1 para que si elije 1 sea 0 
            ...
            const setNestedValue = (object, path, value) => {
                ...
                // 1. Dividir el array en partes usando split
                ...
                // 4. Establece el nuevo valor en la última clave del array
                const finalKey = keys[0]; // al haber sido eliminado por shift el primer elemento, por ejemplo ["addres", "street"] solo queda => [street]
            ...
            let res = await fetch(`${enlace.user}/${id}`, config);
            let data = await res.json();
